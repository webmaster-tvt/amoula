<?php
    ...
    $pofo_portfolio_archive_page_double_grid_position = get_theme_mod( 'pofo_portfolio_archive_page_double_grid_position', '' );
   	...
   	$pofo_show_plus_icon = get_theme_mod( 'pofo_portfolio_archive_page_image_plus_icon', '1' );
    ...
    echo '<ul class="portfolio-grid hover-option8 '.$portfolio_columns.$class_list.'">';
                ...
                echo '<a href="'.esc_url( $portfolio_link ).'"'.$pofo_portfolio_link_target.' class="gallery-link">';
                    echo '<figure>';
                        ...
                        if( has_post_thumbnail() ) {
                            ...
                            echo '</div>';
                            echo '<div class="border-animation"></div>';
                        }

                        echo '<figcaption>';
                            echo '<div class="'.$alignment_class.'portfolio-hover-main pofo-portfolio-archive-page-background">';
                                echo '<div class="'.$vertical_alignment_class.'portfolio-hover-box">';
                                    if( $pofo_show_plus_icon == 1 ):
                                        echo '<div class="pofo-portfolio-archive-page-icon portfolio-icon alt-font font-weight-600 icon-medium">+</div>';
                                    endif;
                                    if($pofo_show_title == 1):
                                        echo '<'.$pofo_title_element_tag.' class="pofo-portfolio-archive-page-title text-extra-large alt-font text-extra-dark-gray margin-5px-bottom'.$pofo_title_text_transform.$pofo_title_dynamic_font_size.'"'.$pofo_title_style_attr.'>'.get_the_title().'</'.$pofo_title_element_tag.'>';
                                    endif;
                                    if($pofo_show_category == 1 && ! empty( $cat_name )):
                                        echo '<p class="pofo-portfolio-archive-page-subtitle no-letter-spacing text-small no-margin-bottom text-medium-gray'.$pofo_category_text_transform.$pofo_subtitle_dynamic_font_size.'"'.$pofo_subtitle_style_attr.'>'.esc_attr( $cat_name ).'</p>';
                                    endif;
                                echo '</div>';
                            echo '</div>';
                        echo '</figcaption>';
