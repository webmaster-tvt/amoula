<?php
    ...
    echo '<ul class="portfolio-grid hover-option2 '.$portfolio_columns.$class_list.'">';
        ...
        while ( have_posts() ) : the_post();
            ...
            ob_start();
                post_class('grid-item '.$double_grid_class.$pofo_animation_style.$pofo_zoom_on_hover.$pofo_portfolio_classes_infinite_scroll);
            ...
            echo '<li '.$pofo_portfolio_archive_classes.$pofo_animation_delay_attr.'>';
                ...
                echo '</div>';
                echo '<a href="'.esc_url( $portfolio_link ).'"'.$pofo_portfolio_link_target.$image_lightbox_title.$image_lightbox_caption.' class="gallery-link">';
                    echo '<figure>';

                        $thumbnail_id   = get_post_thumbnail_id( get_the_ID() );
                        $thumb          = wp_get_attachment_image_src( $thumbnail_id, $pofo_image_srcset_portfolio_archive );
                        $image_url      = ! empty( $thumb['0'] ) ? $thumb['0'] : '';
                        $image_width    = ! empty( $thumb['1'] ) ? $thumb['1'] : '';
                        $image_height   = ! empty( $thumb['2'] ) ? $thumb['2'] : '';

                        if($image_url):
                            echo '<div class="pofo-portfolio-archive-page-background portfolio-img bg-deep-pink">';
                                ...
                                echo '<div class="'.$vertical_alignment_class.'portfolio-hover-box">';
                                    echo '<div class="portfolio-hover-content position-relative">';
                                        if($pofo_show_separator == 1):
                                            echo '<div class="pofo-portfolio-archive-page-separator bg-black margin-four-half-bottom separator-line-horrizontal-medium-light2 display-inline-block"></div>';
                                        endif;
                                        if($pofo_show_title == 1):
                                            echo '<'.$pofo_title_element_tag.' class="pofo-portfolio-archive-page-title text-white line-height-normal alt-font margin-one-half-bottom display-block font-weight-600'.$pofo_title_text_transform.$pofo_title_dynamic_font_size.'"'.$pofo_title_style_attr.'>'.get_the_title().'</'.$pofo_title_element_tag.'>';
                                        endif;
                                        if($pofo_show_category == 1 && ! empty( $cat_name )):
                                            echo '<p class="pofo-portfolio-archive-page-subtitle text-white text-extra-small no-margin-bottom'.$pofo_category_text_transform.$pofo_subtitle_dynamic_font_size.'"'.$pofo_subtitle_style_attr.'>'.esc_attr( $cat_name ).'</p>';
                                        endif;
                                    echo '</div>';
                                echo '</div>';
                            echo '</div>';
