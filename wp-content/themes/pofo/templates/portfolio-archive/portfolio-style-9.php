<?php
    ...
    $pofo_portfolio_archive_page_justified_height = get_theme_mod( 'pofo_portfolio_archive_page_justified_height', '400' );
    $pofo_portfolio_archive_page_gap = get_theme_mod( 'pofo_portfolio_archive_page_gap', '10' );
    ...
    $pofo_portfolio_id = 'portfolio-justified-gallery';

    echo '<div id="' . $pofo_portfolio_id . '" class="justified-gallery justified-gallery-portfolio '.$class_list.'" data-height="'.$pofo_portfolio_archive_page_justified_height.'" data-spacing="'.$pofo_portfolio_archive_page_gap.'">';
        ...
        while ( have_posts() ) : the_post();
            ...
            $image_alt = $pofo_portfolio_archive_page_show_lightbox == 1 && $pofo_show_title == 1 ? ' alt="'.get_the_title().'"' : $image_alt;
            ...
            $pofo_portfolio_archive_classes = get_post_class( $portfolio_classes );
            $pofo_portfolio_archive_classes = ! empty( $pofo_portfolio_archive_classes ) ? ' class="' . implode( ' ', $pofo_portfolio_archive_classes ) . '"' : '';

            echo '<!-- start gallery item -->';
            
                echo '<a href="'.esc_url( $portfolio_link ).'"'.$pofo_portfolio_archive_classes.$image_lightbox_title.$image_lightbox_caption.$pofo_animation_delay_attr.$pofo_portfolio_link_target.'>';
                    ...
                    if( $pofo_show_title == 1 || ( $pofo_show_category == 1 && ! empty( $cat_name ) ) ):
                        echo '<div class="caption pofo-portfolio-archive-page-background">';
                            if($pofo_show_title == 1):
                                echo '<div class="entry-title pofo-portfolio-archive-page-title '.$pofo_title_text_transform.$pofo_title_dynamic_font_size.'"'.$pofo_title_style_attr.'>'.get_the_title().'</div>';
                            endif;
                            if($pofo_show_category == 1 && ! empty( $cat_name )):
                                echo '<span class="pofo-portfolio-archive-page-subtitle '.$pofo_category_text_transform.$pofo_subtitle_dynamic_font_size.'"'.$pofo_subtitle_style_attr.'>'.$cat_name.'</span>';
                            endif;
                        echo '</div>';
                    endif;
                echo '</a>';
