<?php
    ...
    $pofo_portfolio_archive_page_zoom_on_hover = get_theme_mod( 'pofo_portfolio_archive_page_zoom_on_hover', '0' );
    $pofo_zoom_on_hover = ( $pofo_portfolio_archive_page_zoom_on_hover != 1 ) ? ' portfolio-stop-zoom-on-hover': ''; 
    ...
    echo '<ul class="portfolio-grid hover-option5 '.$portfolio_columns.$class_list.'">';
        echo '<li class="grid-sizer"></li>';
        
        $archive_count = 0;
        while ( have_posts() ) : the_post();

            $pofo_animation_delay_attr = ! empty( $pofo_animation_delay ) && ( $archive_count > 0 ) ? ' data-wow-delay="'. ( $pofo_animation_delay * $archive_count ).'ms"' : '';
            ...
            ob_start();
                post_class('pofo-portfolio-archive-page-opacity grid-item '.$double_grid_class.$pofo_animation_style.$pofo_zoom_on_hover.$pofo_portfolio_classes_infinite_scroll);
                    ...
                    echo '<figure>';
                        ...
                        echo '<figcaption>';
                            echo '<div class="'.$alignment_class.'portfolio-hover-main">';
                                echo '<div class="'.$vertical_alignment_class.'portfolio-hover-box pofo-portfolio-archive-page-background">';
                                    echo '<div class="portfolio-hover-content position-relative last-paragraph-no-margin">';
                                        if($pofo_show_separator == 1):
                                            echo '<div class="pofo-portfolio-archive-page-separator bg-deep-pink separator-line-horrizontal-medium-light2 display-inline-block"></div>';
                                        endif;
                                        if($pofo_show_title == 1):
                                            echo '<'.$pofo_title_element_tag.' class="pofo-portfolio-archive-page-title font-weight-600 letter-spacing-1 alt-font text-white margin-5px-bottom display-block'.$pofo_title_text_transform.$pofo_title_dynamic_font_size.'"'.$pofo_title_style_attr.'>'.get_the_title().'</'.$pofo_title_element_tag.'>';
                                        endif;
                                        if($pofo_show_category == 1 && ! empty( $cat_name )):
                                            echo '<p class="pofo-portfolio-archive-page-subtitle text-medium-gray letter-spacing-1 text-extra-small'.$pofo_category_text_transform.$pofo_subtitle_dynamic_font_size.'"'.$pofo_subtitle_style_attr.'>'.esc_attr( $cat_name ).'</p>';
                                        endif;
                                    echo '</div>';
                                echo '</div>';
                            echo '</div>';
