<?php
    ...
    echo '<ul class="portfolio-grid hover-option11 alt-font '.$portfolio_columns.$class_list.$alignment_class.'">';
        ...
        while ( have_posts() ) : the_post();
            ...
            ob_start();
                post_class('grid-item '.$vertical_alignment_class.$double_grid_class.$pofo_animation_style.$pofo_portfolio_classes_infinite_scroll);
            ...
            echo '<li '.$pofo_portfolio_archive_classes.$pofo_animation_delay_attr.'>';
                ...
                echo '<a href="'.esc_url( $portfolio_link ).'"'.$pofo_portfolio_link_target.' class="gallery-link">';
                    ...
                    if( has_post_thumbnail() ) {
                        echo '<div class="portfolio-img bg-extra-dark-gray pofo-portfolio-archive-page-background">';
                            echo get_the_post_thumbnail( get_the_ID(), $pofo_image_srcset_portfolio_archive, $img_attr );
                        echo '</div>';
                    }

                    if($pofo_show_title == 1):
                        echo '<'.$pofo_title_element_tag.' class="pofo-portfolio-archive-page-title portfolio-item-title'.$pofo_title_text_transform.$pofo_title_dynamic_font_size.'"'.$pofo_title_style_attr.'>'.get_the_title().'</'.$pofo_title_element_tag.'>';
                    endif;
