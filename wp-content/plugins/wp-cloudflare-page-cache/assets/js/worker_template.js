...
// Origin Server Error Status Code that will be checked in the worker while fetching from origin server
const ORIGIN_SERVER_ERROR_STATUS_CODES = [ 
...
async function handleRequest(event) {
  ...
  if (!bypassCache) { // bypassCache is still FALSE
    ...
    if (response) { // Cache is present for this request in the CF Edge. Nothing special needs to be done.
      ...
      // Check if the response code we have received from the origin fetch request does not have unwanted status code
      if( ORIGIN_SERVER_ERROR_STATUS_CODES.includes( fetchedResponse.status ) ) {
        return new Response( 
          `Origin Server Error (${fetchedResponse.status}): ${fetchedResponse.statusText}`,
          { 
            status: fetchedResponse.status,
            statusText: `Origin Server Error (${fetchedResponse.status}): ${fetchedResponse.statusText}`
          } 
        )
      }
      ...
      if (!bypassCache) { // bypassCache is still FALSE. Cache the item in the CF Edge
        ...
        if (response.status !== 206 || request?.method !== 'HEAD') {
          ...
          // Check if the response code we have received from the origin fetch request does not have unwanted status code
          if( ORIGIN_SERVER_ERROR_STATUS_CODES.includes( response.status ) ) {
            return new Response( 
              `Origin Server Error (${response.status}): ${response.statusText}`,
              { 
                status: response.status,
                statusText: `Origin Server Error (${response.status}): ${response.statusText}`
              } 
            )
          }
    ...
    // Check if the response code we have received from the origin fetch request does not have unwanted status code
    if( ORIGIN_SERVER_ERROR_STATUS_CODES.includes( bypassedResponse.status ) ) {
      return new Response( 
        `Origin Server Error (${bypassedResponse.status}): ${bypassedResponse.statusText}`,
        { 
          status: bypassedResponse.status,
          statusText: `Origin Server Error (${bypassedResponse.status}): ${bypassedResponse.statusText}`
        } 
      )
    }
